...
\begin{itemize}
	\item	\textit{compact file format}: Soll die Datei in kompakter JSON-Formatierung gespeichert werden?
	...
	\item	\textit{save editing history}: Soll das Änderungsprotokoll (vgl. \ref{befehl-undo}) gespeichert werden, so daß es über Sitzungen hinweg verfügbar ist?
	...
	\item	\textit{save window positions}: Sollen die Positionen der Fenster (Filter, Suche etc.) in der Graphdatei (also projektspezifisch) gespeichert werden?
			(Andernfalls werden die Positionen nur im Browser gespeichert.)
\end{itemize}
...
\subsubsection{Konfiguration bearbeiten: \texttt{config}, \texttt{tagset}, \texttt{makros}, \texttt{metadata}, \texttt{annotators} und \texttt{file}}

Mit diesen Befehlen öffnet man die Fenster für die in Abschnitt \ref{konfiguration} beschriebenen Einstellungen. \texttt{config} für Darstellung und Ebenen, \texttt{tagset} für erlaubte Annotationen, \texttt{makros} für Annotatationsmakros, \texttt{metadata} für Metadaten, \texttt{annotators} für Annotatoren und \texttt{file} für Dateieinstellungen.
...
id              =   "@" (buchstabe | "_") alphanumerisches_zeichen*
...
\begin{itemize}
	...
	\item Suche alle Knoten der Kategorie S, die mindestens einen Knoten mit dem pos-Wert pro direkt dominieren:\\
...
Die \texttt{text}-Klausel besteht aus dem Schlüsselwort \texttt{text}, einer optionalen ID und der Beschreibung eines Textfragments.
...
\paragraph*{Beispiele:}
\begin{itemize}
	\item Suche alle Sätze, in denen das Nomen „Säge“ an dritter Stelle steht:\\
	{\tt
