...
Ein Graph in GraphAnno besteht aus einer Menge von Knoten und gerichteten Kanten. Knoten und Kanten haben Attribute in der Form von Schlüssel-Wert-Paaren, die sowohl der linguistischen Annotation als auch der Strukturierung und Darstellung des Graphen in GraphAnno dienen. Kanten tragen zusätzlich ein \texttt{type}-Attribut.

Ein GraphAnno-Graph ist in Sätze eingeteilt – Einheiten die der Strukturierung und Darstellung dienen, aber natürlich nicht unbedingt (wie auch immer definierten) Sätzen entsprechen müssen. Die Information, zu welchem Satz Knoten und Kanten gehören, ist durch das Attribut \texttt{sentence} repräsentiert.

Tokenknoten sind dadurch gekennzeichnet, daß sie ein Attribut \texttt{token} tragen, das den Tokentext enthält. Die Token eines Satzes sind außerdem in ihrer Reihenfolge durch Kanten mit \texttt{type:t} verbunden (andere Kanten sind \texttt{type:g}). Dies dient vor allem der korrekten Darstellung und der Traversierung; für den Benutzer sind diese Kanten unsichtbar, und er kann nicht auf sie zugreifen.

Für Knoten und Kanten gibt es das Attribut \texttt{cat}, das innerhalb von GraphAnno keine besondere Bedeutung trägt, das aber besonders dargestellt wird, nämlich ohne Schlüssel und stets zuoberst.
...
Ein Satz hat einen Knoten mit dem Attribut \texttt{cat:meta}, der Informationen trägt, die den ganzen Satz betreffen. Hierbei kann es sich z.B. um Angaben von Quelle, Medium, Sprecher etc. handeln. Der Inhalt dieses Knotens wird im hellgrauen Bereich unter dem Annotationsgraphen und unter dem Text des Satzes (blau dargestellt) in schwarzer Schrift angezeigt.

GraphAnno bietet auch die Möglichkeit, Knoten und Kanten unterschiedlichen Ebenen zuzuordnen. Zur Zeit sind zwei Ebenen implementiert: Eine funktionale/semantische Ebene und eine formale/syntaktische. Zugehörigkeit eines Knoten oder einer Kante zu ersterer wird durch das Attribut \texttt{f-layer:t} wiedergegeben, Zugehörigkeit zu letzterer durch \texttt{s-layer:t}. Elemente, die dem \texttt{f-layer} angehören sind grün dargestellt, Elemente, die dem \texttt{s-layer} angehören, blau, Elemente, die beiden angehören, schwarz und Elemente, die keiner Ebene angehören, grau. Token sind keiner Ebene zugeordnet und werden schwarz dargestellt.

%Schließlich tragen Kanten noch das Attribut \texttt{edgetype}, das – in Anlehnung an andere Korpusformate – den Wert \texttt{p} (für \textit{primär}) oder \texttt{s} (für \textit{sekundär}) annehmen kann. Dieses Attribut dient der Darstellung, und zwar insofern als Kanten mit dem Attribut \texttt{edgetype:p} nach Möglichkeit kürzer dargestellt werden. Die Werte für \texttt{edgetype} sind an die Ebene gekoppelt: Kanten, die der \texttt{s-ebene} angehören sind \texttt{edgetype:p}, alle anderen \texttt{edgetype:s}.

\paragraph*{Die speziellen Attribute im Überblick:}
\begin{itemize}
	\item {\tt sentence}: Satz, dem Knoten und Kanten angehören
	\item {\tt token}: enthält den Text von Tokenknoten
	\item {\tt cat}: privilegiertes Attribut
	\item {\tt f-layer}: Zugehörigkeit zur funktionalen/semantischen Ebene
	\item {\tt s-layer}: Zugehörigkeit zur formalen/syntaktischen Ebene
	%\item {\tt edgetype}: Kantentyp zur unterschiedlichen Darstellung
\end{itemize}
...
\subsubsection{Neuen Satz anlegen: \texttt{ns}}

Mit \texttt{ns} – gefolgt von mit Leerzeichen getrennten anzulegenden Namesräumen – werden ein oder mehrere neue Sätze angelegt. Dabei werden neue Meta-Knoten mit dem entsprechenden \texttt{sentence}-Attribut erstellt. Anschließend wird sofort in den ersten angegebenen Satz gewechselt.


\subsubsection{Satz löschen: \texttt{del}}

Mit dem Befehl \texttt{del} werden alle Elemente des gegenwärtig angezeigten Satzes gelöscht.


\subsubsection{Gehe zu: \texttt{s}}

Um von Satz zu Satz zu navigieren dient (neben dem Aufklappfeld, das auf Änderungen reagiert) der Befehl \texttt{s}, gefolgt vom Namen des anzuzeigenden Satzes.
...
\subsubsection{Text importieren}

Texte können mit dem Befehl \texttt{import} importiert werden.
...
\subsubsection{Toolboxdaten importieren}
...
\subsubsection{Neuer Knoten: \texttt{n}}
...
Des weiteren kann die Ebene angegeben werden, der der neue Knoten angehören soll. Dazu dienen die auch für den Befehl \texttt{l} (siehe \ref{l}) geltenden Kürzel \texttt{f}, \texttt{s} und \texttt{fs}. Die Angabe der Ebene hat zudem die Wirkung eines Schalters, der wie der Befehl \texttt{l} die Ebene für die folgenden Operationen setzt.
...
Mit dem Befehl \texttt{l} wird, alternativ zum Aufklappfeld, die Ebene gesetzt, in der sich die nachfolgend erstellten Elemente befinden sollen. \texttt{f} steht hier für die funktionale/semantische Ebene, \texttt{s} für die formale/morphosyntaktische, und \texttt{fs} für beide.
