...
The key F8 opens the configurations window for the currently loaded graph.
This window provides the possibility to configure the layers of the graph and its visualization.
...
\subsubsection{Permitted annotations}\label{erlaubteannotationen}

In the window open by the key F10, you can specify which annotation are permitted for the nodes and edges of the graph, i.e., you can define the graph-specific tagset.
...
If you enter keys, only these keys are allowed for annotations.
...
The key F9 open the window, where you can enter an arbitrary number of keys with a text as corresponding value.
...
\begin{center}
	\begin{tabular*}{\textwidth}{ll}
		...
		\midrule
			...
			F8 & show/hide layer settings window\\
			F9 & show/hide metadata window\\
			F10 & show/hide tagset window\\
		\bottomrule
	\end{tabular*}
\end{center}
