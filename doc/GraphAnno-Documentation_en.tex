...
\subsubsection{Edit configurations: \texttt{config}, \texttt{tagset} and \texttt{metadata}}

These commands open the windows for the settings that were described in Section \ref{konfiguration}.
\texttt{config} for layers and visualization, \texttt{tagset} for permitted annotations / tagset and \texttt{metadata} for metadata.
...
string              =   character_not_special+
                        """ character* """
annotation_shortcut =   alphanumeric_character+
layer_shortcut      =   alphanumeric_character+
\end{lstlisting}
\end{framed}
...
\paragraph*{Command \texttt{g} in modified BNF:}
~
\begin{framed}
\begin{lstlisting}
...
Der Befehl \texttt{h}/\texttt{c} funktioniert wie \texttt{g}/\texttt{p}, mit dem Unterschied, daß anstelle eines Mutterknotens ein neuer gemeinsamer Tochterknoten erstellt wird.
        ...
        %ul
          %li h n2 n4 typ:R
\paragraph*{Command \texttt{h} in modified BNF:}
~
\begin{framed}
\begin{lstlisting}
...
\paragraph*{Command \texttt{di}/\texttt{do} in modified BNF:}
~
\begin{framed}
\begin{lstlisting}
...
\paragraph*{Commande \texttt{t}, \texttt{tb}, \texttt{ta} in modified BNF:}
~
\begin{framed}
\begin{lstlisting}
befehl_t        =   "t " wörter
